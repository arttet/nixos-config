...
hardware profile. The tuning module provides conservative real-hardware runtime
defaults through `platform.tuning`. The `vm` profile imports `workstation` and
keeps QEMU-only settings out of the hardware profile.
